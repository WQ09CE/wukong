...
def find_source_dir() -> Path:
    ...
    Search order:
    1. Current working directory's .wukong/ (development mode)
    2. Package's built-in assets/ (packaged mode)
    ...
    # 2. Package built-in assets (packaged mode)
    ...
    raise FileNotFoundError(
        ...
        "Run this command from a directory containing .wukong/ "
        "or install the full wukong-cli package."
    )
...
def do_install(target: str, dry_run: bool = False, verbose: bool = False) -> int:
    ...
    if rules_lite_src.exists():
        ...
        _copy_files(rules_lite_src, rules_dst, "*.md", "Core rules (lite)", dry_run, verbose)
    else:
    ...
    if rules_src.exists():
        ...
        _copy_rules_extended(rules_src, rules_ext_dst, dry_run, verbose)

    # Copy skills
    skills_src = source / "skills"
    if skills_src.exists():
        skills_dst = target_path / ".claude/skills"
        _copy_files(skills_src, skills_dst, "*.md", "Skills", dry_run, verbose)

    # Copy commands
    commands_src = source / "commands"
    if commands_src.exists():
        commands_dst = target_path / ".claude/commands"
        _copy_files(commands_src, commands_dst, "*.md", "Commands", dry_run, verbose)
    ...
    if templates_src.exists():
        ...
        if dry_run:
            ...
            console.print(f"  [green][ok][/green] Templates")
    ...
    if ctx_templates_src.exists():
        ...
        if dry_run:
            ...
            console.print(f"  [green][ok][/green] Context templates")
