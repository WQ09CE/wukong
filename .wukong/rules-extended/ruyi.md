...
## Trigger Mode (触发模式) - 显式触发

> **重要**: 上下文管理通过**显式命令**触发，不自动执行。
> Claude Code 无法暴露上下文使用率，因此无法实现自动触发。

### 可用命令

| 命令 | 动作 | 输出 |
|------|------|------|
| `/wukong 内观` | 深度反思 + 提取锚点 | 内观报告 + 三态摘要 |
| `/wukong 压缩` | 生成缩形态摘要 | 🔸 缩形态 (<500字) |
| `/wukong 存档` | 保存完整上下文 | 文件保存到 `sessions/` |
| `/wukong 加载 {name}` | 加载历史上下文 | 恢复会话状态 |
| `/wukong 锚点` | 显示所有锚点 | 锚点列表 |
...
| 限制 | 原因 |
|------|------|
| 无法获取上下文使用率 | Claude Code API 不暴露 token 计数 |
| 无法主动中断 | 只能在回复时执行，不能在对话中间插入 |
| 无法修改历史消息 | DCP 的"修剪"在技术上无法实现 |

**Claude Code 内置能力**：
- 大输出自动保存到磁盘 (v2.1.2+)
- 上下文满时自动 summarization
- 这些是系统级的，Wukong 协议配合但无法控制
