...
### 强制委派检查 (Mandatory Delegation)

**探索类任务 → 必须委派眼分身(后台)**:
```
触发词: 审视/探索/查看/了解/分析/搜索
动作: 立即召唤眼分身(run_in_background=true)
返回: "已派眼分身探索，你可以继续问我其他问题"
```

**实现类任务 → 必须委派斗战胜佛(后台)**:
```
触发词: 改进/实现/修复/重构/添加功能/开发
动作: 召唤斗战胜佛(run_in_background=true)
返回: "斗战胜佛正在执行，你可以继续问我其他问题"
```
