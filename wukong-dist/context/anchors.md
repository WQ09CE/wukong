...
### [P003] hui-extract.py 误提取对话片段为锚点
**时间**: 2026-01-13
**状态**: Resolved (已修复代码)
**症状**: PreCompact Hook 把普通对话内容误提取为 D005/D006/P003 锚点
**根因**:
- 关键词匹配太宽泛（只要包含"决定/采用/Use"就提取）
- 门槛检查太宽松（默认 impact=True 导致几乎全部通过）
- 标题提取简单取第一行，对话开头被当作标题
**解决**:
- 增加 `_is_valid_anchor_content()` 内容验证函数
- 排除对话片段（检测常见对话开头）
- 要求结构化关键词（Context/Decision/Impact 等）
- 严格化门槛：impact 需配合内容长度，reusable 只对 problem 类型生效
**预防**:
- 提取逻辑增加格式验证
- 门槛检查基于内容分析动态设置
**Tags**: #bug #hui-extract #anchor-quality
